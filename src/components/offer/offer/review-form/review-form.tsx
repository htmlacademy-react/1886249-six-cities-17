...
function ReviewForm() {
  ...
  const postStatus = useAppSelector(
    offerFullSElectors.selectPostReviewRequestStatus,
  );
  ...
  if (review.comment.length >= Number(ReviewLength.Min) && review.rating !== 0) {
    ...
    >
      ...
      {postStatus === RequestStatus.Failed && (
        <p className='reviews__error'>
					Sorry, an error occured. Try one more time.
        </p>
      )}
